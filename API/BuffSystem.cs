...
namespace BloodyCore.API
{
    ...
    public class BuffSystem
    {
        ...
        public static bool BuffNPC(Entity character, Entity user, PrefabGUID buff, int duration = DEFAULT_DURATION)
        {
            ...
            var des = BloodyCore.Systems.DebugEventsSystem;
            ...
            if (!BuffUtility.TryGetBuff(BloodyCore.Systems.EntityManager, character, buff, out Entity buffEntity))
            {
                des.ApplyBuff(fromCharacter, buffEvent);
                if (BuffUtility.TryGetBuff(BloodyCore.Systems.EntityManager, character, buff, out buffEntity))
                {
        ...
        {
            var buffs = BloodyCore.Systems.EntityManager.GetBuffer<BuffBuffer>(player);
            ...
            foreach (var buff in buffs)
            {
                ...
                if (shouldRemove)
                {
                    DestroyUtility.Destroy(BloodyCore.Systems.EntityManager, buff.Entity, DestroyDebugReason.TryRemoveBuff);
                }
            }
            var equipment = player.Read<Equipment>();
            if (!equipment.IsEquipped(Prefabs.Item_Cloak_Main_ShroudOfTheForest, out _) && BuffUtility.HasBuff(BloodyCore.Systems.EntityManager, player, new PrefabGUID(476186894)))
            {
        ...
        {
            if (BuffUtility.TryGetBuff(BloodyCore.Systems.EntityManager, Character, buffGUID, out var buffEntity))
            {
                DestroyUtility.Destroy(BloodyCore.Systems.EntityManager, buffEntity, DestroyDebugReason.TryRemoveBuff);
            }
        }
    }
}
